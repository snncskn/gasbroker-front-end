...
import { UsersList, UsersPagination, UsersRoles } from 'app/modules/admin/apps/users/users.types';
...
export class UsersService {
    // Private
    private _pagination: BehaviorSubject<UsersPagination | null> = new BehaviorSubject(null);
    private _user: BehaviorSubject<UsersList | null> = new BehaviorSubject(null);
    private _users: BehaviorSubject<UsersList[] | null> = new BehaviorSubject(null);
    private _roles: BehaviorSubject<UsersRoles[] | null> = new BehaviorSubject(null);
    ...
    getUsers(page: number = 0, size: number = 10, sort: string = 'userName', order: 'asc' | 'desc' | '' = 'asc', search: string = ''):
        ...
        return this._httpClient.post<{ pagination: UsersPagination; users: UsersList[] }>(url, {
            queryParams: {
                pageNumber: '' + page,
                pageSize: '' + size,
                sortField: sort,
 ...
 */
    ...
    createUser(): Observable<any> {
        ...
        let tmp = { userName: '', firstName: '', pass: '', full_name: '', email: '', mobilePhone: '', gender: '', birthday: '', color: 'bg-teal-500' };
        let url = `${environment.url}/users/save`;
        return this.users$.pipe(
            take(1),
            switchMap(users => this._httpClient.post<any>(url, { user: tmp }).pipe(
                map((newUser) => {
    ...
    createUserSingUp(user:any): Observable<any> {
        ...
        return this.users$.pipe(
            take(1),
            switchMap(users => this._httpClient.post<any>(url, { user: user }).pipe(
                map((newUser) => {
    ...
    updateUser(id: string, user: UsersList): Observable<UsersList> {
        ...
        return this.users$.pipe(
            take(1),
            switchMap(users => this._httpClient.put<any>(url, {
                id,
                user
            }).pipe(
                map((updatedUser) => {
    ...
    deleteUser(id: string): Observable<boolean> {
        ...
        return this.users$.pipe(
            take(1),
            switchMap(users => this._httpClient.delete(url, { params: { id } }).pipe(
                map((isDeleted: any) => {
